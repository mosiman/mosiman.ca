...
![A list of 20 online users is presented to us, and it also seems to change it up on refreshes](/img/mal-scraping/mal_users.PNG)
...
![Using the "inspect" tool in the browser dev tools to help nab particular elements.](/img/mal-scraping/userstats.png)
...
![When we scroll to the bottom, a request is made to the server with parameters `offset` and `status`. Dig around, and you'll find out that `status` identifies 'All anime' vs 'Currently Watching' vs 'Completed' etc](/img/mal-scraping/xhr.png)
...
![json data from the API](/img/mal-scraping/animelist3_cropped.PNG)
